...
def test_model_origin(net, data_loader, save_output=False, save_path=None, test_fixed_size=-1, test_batch_size=1,
    ...
    if args.save_txt:
        ...
        record = open(save_txt_path+'/DLA_loc_test_thr_{:.02f}.txt'.format(args.det_thr), 'w+')
    ...
    for i, blob in enumerate(data_loader.get_loader(test_batch_size, num_workers=args.num_workers)):
        ...
        if save_output:
            ...
            dgen.save_density_map(gt_dens.squeeze(), save_path, fname[0].split('.')[0] + "_1_gt.jpg", gt_count)
        ...
        if args.save_txt:
            ...
            if i == len(data_loader) - 1:
                record.write('{filename} {pred:d}{loc_str}'.format(filename=fname[0].split('.')[0], pred=et_count,
                                                                   loc_str=loc_str))
                record2.write('{filename} {pred:0.2f}'.format(filename=fname[0].split('.')[0],pred=float(et_count_dm)))
            else:
                record.write('{filename} {pred:d}{loc_str}\n'.format(filename=fname[0].split('.')[0],pred=et_count,
                                                                     loc_str=loc_str))
                record2.write('{filename} {pred:0.2f}\n'.format(filename=fname[0].split('.')[0],pred=float(et_count_dm)))
...
if __name__ == '__main__':
    ...
    data_loader_test = CreateDataLoader(args, phase='test_unlabel')
    ...
    for model_file in model_files:
        ...
        if args.test_batch_size != 1 or args.test_fixed_size != -1:
            ...
            test_mae, test_mse, detail = test_model_fun(net, data_loader_test, args.save_output, \
                                                           output_dir, test_fixed_size=args.test_fixed_size,
                                                           test_batch_size=args.test_batch_size, \
                                                           gpus=args.gpus, args=args)
        ...
        if args.is_search_thr:
            ...
            # log_text = 'TEST EPOCH: %s, MAE: %.2f, MSE: %0.2f\n' % (epoch, test_mae, test_mse)
