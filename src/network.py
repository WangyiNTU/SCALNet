...
def decode(hm, thr=0.3):
    scores = hm['hm']
    batch, cat, height, width = scores.size()

    scores = scores.view(batch, -1)
    scores_mask = scores.view(batch, -1) > thr
    topk_inds = torch.nonzero(scores_mask)
    topk_scores = scores[scores_mask]

    topk_ys = (topk_inds[:,1] / width).int().float()
    topk_xs = (topk_inds[:,1] % width).int().float()

    scores_o = {}
    for head in hm:
        if head == 'hm':
            continue
        scores_o[head] = hm[head].view(batch,-1)[scores_mask]

    detections = torch.stack([topk_inds[:,0].float(), topk_xs, topk_ys, topk_scores,
                            scores_o['hm0'], scores_o['hm1'], scores_o['hm2'],
                            scores_o['hm3'], scores_o['hm4']], dim=-1)

    return detections

def det_decode(hm):
    for head in hm:
        hm[head] = hm[head].sigmoid_()
    # rescale hmX to the size of hm, and filtering
    hm_size = hm['hm'].size()[2:]
    for head in hm:
        if head == 'hm':
            hm['hm'] = _nms(hm['hm'])
            continue
        hm[head] = F.interpolate(hm[head], size=hm_size, mode='bilinear')
        # hm[head] = max_filter(hm[head])

    detections = decode(hm, thr=0.3)
    return detections
